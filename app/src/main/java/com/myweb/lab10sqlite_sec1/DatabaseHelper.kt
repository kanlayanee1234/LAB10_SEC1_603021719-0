...
class DatabaseHelper (context: Context): SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    override fun onCreate(db: SQLiteDatabase?) {
        val CREATE_TABLE :String = "CREATE TABLE $TABLE_NAME"+ "($COLUMN_ID TEXT PRIMARY KEY, $COLUMN_NAME TEXT," +
                "$COLUMN_AGE Integer)"
    ...
    fun insertStudent(std : Student): Long{
        ...
        val suck = db.insert(TABLE_NAME, null, values)
        db.close()
        return suck
    ...
    fun updateStudent(std: Student):Int{
        ...
        val suck = db.update(TABLE_NAME, values,"$COLUMN_ID = ?", arrayOf(std.id))
        return suck
    }

    fun delete(std_id: String):Int{
        val db = writableDatabase
        val suck = db.delete(TABLE_NAME,"$COLUMN_ID = ?", arrayOf(std_id))
        db.close()
        return suck
